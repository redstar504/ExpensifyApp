...
class CommentLength extends PureComponent {
    constructor(props) {
        ...
        this.updateCommentLength = debounce(() => {
            const length = ReportUtils.getCommentLength(this.props.comment);
            this.setState({commentLength: length});
            this.props.onExceededMaxCommentLength(length > CONST.MAX_COMMENT_LENGTH);
        }, 500);
    }
